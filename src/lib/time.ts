...
export function parseCampusLocal(s: string): Date | null {
  ...
  if (!m) return null;
  const [year, month, day, hour, min] = [+m[1], +m[2], +m[3], +m[4], +m[5]];
  // Reject out-of-range parts (e.g. month 13, hour 25) that Date.UTC would silently roll over.
  if (month < 1 || month > 12 || day < 1 || day > 31 || hour > 23 || min > 59) {
    return null;
  }
  const utcMs = Date.UTC(year, month - 1, day, hour, min) - 7 * 60 * 60 * 1000;
